...
async def start_handler(message: Message):
    ...
    await message.answer("Выберите раздел", keyboard=new_kb.get_json(), dont_parse_links=True)
...
async def back_handler(message: Message):
    ...
    if info:
        ...
        await message.answer(to["text"], keyboard=new_kb.get_json(), attachment=to.get("image", ""), dont_parse_links=True)
...
async def main_handler(message: Message):
    ...
    await message.answer(Config.INFO[message.text]["text"], keyboard=new_kb.get_json(), dont_parse_links=True)
...
async def additional_handler(message: Message):
    ...
    if info:
        ...
        await message.answer(data["text"], keyboard=new_kb.get_json(), attachment=data.get("image", ""), dont_parse_links=True)
    else: await error_handler(message)
